...
<script type="text/javascript">
    ...
    $('.j-send').click(function(){
        $('.j-send-form [name="order"]').val($(this).attr('data-id'));
        $('.j-send-form').show();
    });
...
@foreach(auth()->user()->dealer_orders()->where('type', 1)->orderBy('created_at', 'DESC')->get() as $order)
    <div class="f_Ordercontent">
        ...
        <div class="f_state_b b_b_e5 fs_28 prl24 ptb_13 clearfix">
            ...
            @if($order->status <= 100)
                <div class="fr">
                    @if($order->status == 3)<a href="/mobile/user/order/make/{{ $order->trade_id }}" class="f_rightbottom js_queding fl m_l_24 c_999 j-make" data-id="{{ $order->trade_id }}">制作完毕</a>@endif
                    @if($order->status == 5)<a href="javascript:void(0);" class="f_rightbottom js_queding fl m_l_24 c_999 j-send" data-id="{{ $order->trade_id }}">发货完毕</a>@endif
...
<div class="mask hide j-send-form">
    <input type="hidden" name="order">
    <div class="allfabubox">
        <div class="Masterworker b_b_e5">
            <div>
                <input type="text" name="company" placeholder="快递公司" class="">
            </div>
            <div>
                <input type="text" name="number" placeholder="快递单号" class="">
            </div>
        </div>
        <div class="fabottom">
            <a class=" c_999 fs_36 b_r_e5 j-send-submit" href="javascript:void(0);">确定</a>
            <a class=" c_9b34 fs_36 j-call" href="javascript:void(0);" onclick="$('.j-send-form').hide();">取消</a>
        </div>
    </div>
</div>
