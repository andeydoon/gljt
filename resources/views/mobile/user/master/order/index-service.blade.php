...
@foreach(auth()->user()->master_orders()->where('type', 2)->orderBy('created_at', 'DESC')->get() as $order)
    <div class="f_Ordercontent">
        ...
        <div class="pName prl24 pbt24 clearfix b_b_e5">
            ...
            <div class="ptext fl m_l_15 posr">
                ...
                @if($order->status >= 2 && $order->status <= 100)
                    <a class="posa programme" href="/mobile/user/order/service_scheme/{{ $order->trade_id }}">查看方案</a>
                @endif
            </div>
        </div>
        @if($order->status <= 100)
        ...
        <div class="f_state_b b_b_e5 fs_28 prl24 ptb_13 clearfix">
            ...
            @if($order->status <= 100)
                <div class="fr">
                    @if($order->status == 1)<a href="/mobile/user/order/service_scheme_create/{{ $order->trade_id }}" class="f_rightbottom js_queding fl m_l_24 c_999 ">编辑方案</a>@endif
