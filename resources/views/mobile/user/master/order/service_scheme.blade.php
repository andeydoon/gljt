...
<div class="allinfo w702">
    <ul class="position fs_30 c_333">
        ...
        <li>{{ $order->service_scheme->content }}</li>
    </ul>

    <ul class="position fs_30 c_333">
        @foreach(unserialize($order->service_scheme->parts) as $part)
            <li class="lastli clearfix">
                <span class="t_a_l">名称：{{ $part['name'] }}</span>
                <span class="t_a_c">单价：{{ $part['price'] }}</span>
                <span class="t_a_r">数量：{{ $part['quantity'] }}</span>
            </li>
        @endforeach
    </ul>
    ...
    <ul class="Price_product fs_30 c_333">
        ...
        <li>人工：{{ $order->service_scheme->cost_labor }}元</li>
