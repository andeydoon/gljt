...
<div class="f_topnav p_t_15 p_b_15 b_b_e5">
    <nav class="f_nv c_9425 clearfix fs_26">
        ...
        <a href="/mobile/user/publish?service">服务</a>
    </nav>
</div>
<!-- navtop -->
...
@foreach(auth()->user()->member_orders()->where('type', 1)->orderBy('created_at', 'DESC')->get() as $order)
    <div class="f_Ordercontent">
        ...
        <div class="pName prl24 pbt24 clearfix b_b_e5">
            ...
            <div class="ptext fl m_l_15 posr">
                ...
                @if($order->status >= 2 && $order->status <= 100)
                    <a class="posa programme" href="/mobile/user/publish/custom_scheme/{{ $order->trade_id }}">查看方案</a>
                @endif
            </div>
        </div>
        @if($order->status <= 100)
            ...
            <div class="Progressbar clearfix b_b_e5">
                <ul class="custom" clearfix>
                    ...
                    <li class="fs_24{{ $order->status>4 ? ' Probar_active' : '' }}">
                        ...
                        <span class="option_bottom">尾款</span>
                    </li>
        ...
        <div class="f_state_b b_b_e5 fs_28 prl24 ptb_13 clearfix">
            ...
            @if($order->status <= 100)
                <div class="fr">
                    @if($order->status == 2)<a href="/mobile/user/publish/pay_part1/{{ $order->trade_id }}" class="f_rightbottom js_queding fl m_l_24 c_999">付定金</a>@endif
                    @if($order->status == 4)<a href="/mobile/user/publish/pay_part2/{{ $order->trade_id }}" class="f_rightbottom js_queding fl m_l_24 c_999">付款</a>@endif
                    ...
                    @if($order->status == 6)<a href="javascript:void(0);" class="f_rightbottom js_queding fl m_l_24 c_999 js-shouhuo_btn" data-id="{{ $order->trade_id }}">确认收货</a>@endif
                    @if($order->status == 6)<a href="javascript:void(0);" class="f_rightbottom js_queding fl m_l_24 c_999">查看物流</a>@endif
                    @if($order->status == 7)<a href="/mobile/user/publish/comment/{{ $order->trade_id }}" class="f_rightbottom js_queding fl m_l_24 c_999">确认安装</a>@endif
                </div>
            @endif
        </div>
    </div>
