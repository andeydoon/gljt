...
<div class="z_w82p fr z_minh640">
    ...
    <div class="z_mb10">
        ...
        <!--服务-->
        @foreach(auth()->user()->master_orders()->where('type', 2)->orderBy('created_at', 'DESC')->get() as $order)
                ...
                <div class="bar-LightGrey z_border z_mb10">
                    ...
                    <div class="z_border_b z_p20 clearfix posr">
                        ...
                        @if($order->status >= 2 && $order->status <= 100)
                            <a href="/user/order/service_scheme/{{ $order->trade_id }}" title="查看方案" class="z_btn bar-white z_font12 z_w74 z_ptb3-i posa z_right-20 z_bottom-20 z_index-8 z_border_rn z_color_3 z_border" target="_blank">查看方案</a>
                        @endif
                    </div>
                    @if($order->status <= 100)
                        ...
                        <div class="z_p10 tr">
                            @if($order->status == 1)<a href="/user/order/service_scheme_create/{{ $order->trade_id }}" class="z_btn bar-auburn z_font12 z_w68 z_ptb3-i z_ml10">编辑方案</a>@endif
                            ...
                            {{--@if($order->status >= 4)<a href="javascript:void(0);" class="z_btn bar-auburn z_font12 z_w68 z_ptb3-i z_ml10">查看评论</a>@endif--}}
                        </div>
                    @endif
                </div>
            </div>
