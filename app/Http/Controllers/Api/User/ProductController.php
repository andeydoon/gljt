...
class ProductController extends Controller
{
    ...
    public function update(Request $request, $id)
    {
        ...
        if (!$product = $user->products()->where('id', $id)->first()) {
    ...
    public function patch(Request $request, $id)
    {
        ...
        if ($request->status == 0) {
            ...
            if (in_array($product->status, [-1, 2])) {
                return response()->json(['status' => 'error', 'message' => '产品不在未审状态']);
            }
        }
        ...
        if ($request->status == 1) {
            //不在下架状态
            if ($product->status != 3) {
                return response()->json(['status' => 'error', 'message' => '产品不在下架状态']);
            }
        }

        //拒审
        if ($request->status == 2) {
            return response()->json(['status' => 'error', 'message' => '无权拒审']);
        }
        ...
        if ($request->status == 3) {
            //不在上架状态
            if ($product->status != 1) {
                return response()->json(['status' => 'error', 'message' => '产品不在上架状态']);
            }
        }
