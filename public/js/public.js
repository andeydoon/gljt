...
function sendAllProvinceAjax() {
	$.ajax({
		type: "get",
		url: '/项目/广铝/广铝pc/js/queryAllProvinces.js',
		async: false,
		dataType: "json",
		success: function(data) {
...
function sendAllCitiesAjax() {
	$.ajax({
		type: "get",
		url: '/项目/广铝/广铝pc/js/queryCities.js',
		async: false,
		dataType: "json",
		success: function(data) {
...
function sendAllCountiesAjax()
 {
	$.ajax({
		type: "get",
		url: "/项目/广铝/广铝pc/js/queryAllAreas.js",
		async: false,
		dataType: "json",
		success: function(data) {
			allAreas = data.areas;
...
function addrInputAll(i) {
	...
	var _html ='<span>'+c + "-" + a+'<i class="z_plr5 z_ptb3 z_cursor z_color_red z_ml10 js-z_off">x</i>'+'</span>';
	$(".js-z_quyu").append(_html);
	delet2();
	console.log(rtn);//输出
	$(".backifname").hide();
